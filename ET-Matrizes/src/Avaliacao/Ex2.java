	...
	public static void main(String[] args) {
		...
		for (int l = 0; l < 3; l++) {
			for (int c = 0; c < 3; c++) {
				...
				// CODIGO LINDO DE BONITO, TOMAR NO MEU CU, TO ORGULHOSO, O QUE É NÚMERO É PQ É
				// PRA SER ESTATICO MESMO, TOMAR NO CU.
				// chorei fazendo essa merda. Ah é, L é pra ser altera só quando descer, e C é
				// pra ser alterado constantemente.
		...
		for (int i = 0; i < 3; i++) {
			...
			if (i == 0)
				dP = p2[0][i] * p2[1][i + 1] * p2[2][2];
			if (i == 1)
				dP += p2[0][i] * p2[1][i + 1] * p2[2][i - 1];
			if (i == 2)
				dP += p2[0][i] * p2[1][i - 2] * p2[2][i - 1];
			// parte negativa da determinante
			if (i == 0)
				dN = (p2[0][2] * p2[1][i + 1] * p2[2][i]) * -1;
			if (i == 1)
				dN += (p2[0][i - 1] * p2[1][i + 1] * p2[2][i]) * -1;
			if (i == 2)
				dN += (p2[0][i - 1] * p2[1][i - 2] * p2[2][i]) * -1;

		}
		determinante = dP + dN; // gerar determinante
		w += "Determinante =  " + determinante; // armazenar a determinante na resposta
		JOptionPane.showMessageDialog(null, w, "Resposta", -1); // RESPOSTA FINAL, n tem o calculo da determinante pq
																// preguiça, mas tem a determinante
	}
}
