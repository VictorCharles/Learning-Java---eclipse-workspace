...
public class CaixaEletronico {
	...
	public static void main(String[] args) {
		...
		/*
		 * Preencher Vetor CxEl com a quantidade de cada cedula
		 */

		for (int y = 0; y < 6; y++) {
			CxEl[y] = Integer
					.parseInt(JOptionPane.showInputDialog(null, "Digite a quantidade de Cedulas de R$ " + Valor[y],
							"Procedimento Interno - Abastecendo Caixa Eletronico", 3));
		...
		JOptionPane.showMessageDialog(null, "Caixa Eletronico abastecido com as seguintes quantidades\n"
				...
				+ " = R$ " + (float) (CxEl[1] * Valor[1]) + "\nR$  10,00 x " + CxEl[2] + " = R$ "
				+ (float) (CxEl[2] * Valor[2]) + "\nR$  20,00 x " + CxEl[3] + " = R$ " + (float) (CxEl[3] * Valor[3])
				+ "\nR$  50,00 x " + CxEl[4] + " = R$ " + (float) (CxEl[4] * Valor[4]) + "\nR$ 100,00 x " + CxEl[5]
				+ " = R$ " + (float) (CxEl[5] * Valor[5]) + "\n\nTotal Abastecido R$ " + valorcx,
				"Procedimento Interno - Caixa Abastecido", 1);
		/* Procedimento Externo - Saque do Caixa Eletronico */

		while (x) {
			String mens = "Escolha uma opçao de Saque:\n";
			float valorsq = Integer.parseInt(
					JOptionPane.showInputDialog(null, "Qual valor deseja sacar?", "Procedimento Externo - Saque", 3));
			float valorint = valorsq;
			if (valorsq > valorcx) {
				JOptionPane.showMessageDialog(null, "Nao ha valor disponivel para essa quantia\nTente outro Caixa",
						"Procedimento Externo - Saque", 0);
				continue;
			}
			int[] Sq1 = new int[6];
			mens += "\nOpçao 1:\n";
			...
			if (Sq1[5] != 0)
				mens += Sq1[5] + " notas de R$ 100,00\n";
			if (Sq1[4] != 0)
				mens += Sq1[4] + " notas de R$ 50,00\n";
			if (Sq1[3] != 0)
				mens += Sq1[3] + " notas de R$ 20,00\n";
			if (Sq1[2] != 0)
				mens += Sq1[2] + " notas de R$ 10,00\n";
			if (Sq1[1] != 0)
				mens += Sq1[1] + " notas de R$ 5,00\n";
			if (Sq1[0] != 0)
				mens += Sq1[0] + " notas de R$ 2,00\n";
			...
			if (Sq2[5] != 0)
				mens += Sq2[5] + " notas de R$ 100,00\n";
			if (Sq2[4] != 0)
				mens += Sq2[4] + " notas de R$ 50,00\n";
			if (Sq2[3] != 0)
				mens += Sq2[3] + " notas de R$ 20,00\n";
			if (Sq2[2] != 0)
				mens += Sq2[2] + " notas de R$ 10,00\n";
			if (Sq2[1] != 0)
				mens += Sq2[1] + " notas de R$ 5,00\n";
			if (Sq2[0] != 0)
				mens += Sq2[0] + " notas de R$ 2,00\n";
			...
			JOptionPane.showMessageDialog(null,
					"Caixa Eletronico abastecido com as seguintes quantidades\n" + "\nR$   2,00 x " + CxEl[0] + " = R$ "
							+ (float) (CxEl[0] * 2) + "\nR$   5,00 x " + CxEl[1] + " = R$ " + (float) (CxEl[1] * 5)
							+ "\nR$  10,00 x " + CxEl[2] + " = R$ " + (float) (CxEl[2] * 10) + "\nR$  20,00 x "
							+ CxEl[3] + " = R$ " + (float) (CxEl[3] * 20) + "\nR$  50,00 x " + CxEl[4] + " = R$ "
